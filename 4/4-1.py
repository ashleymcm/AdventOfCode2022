...
def sectionFullyContainsOther(pair):
...
def countNumberOfFullyContainedSections(assignment_pairs):
  total = 0
  for pair in assignment_pairs:
    if sectionFullyContainsOther(pair):
      total += 1
  return total
...
with open(os.path.join(dirname, "input.txt")) as assignment_pair_list:
  ...
  print(countNumberOfFullyContainedSections(assignment_pairs))